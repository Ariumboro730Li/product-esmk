    ...
    <script>
        ...
        async function getPerusahaanData(id) {
            ...
            const getDataRest = await CallAPI(
                    'GET',
                    `{{ url('') }}/api/internal/admin-panel/perusahaan/detail`, {
                        id: id,
                    }
                )
        ...
        async function getListPengajuanSmk(limit = 10, page = 1, ascending = 0, search = '', id = referenceId) {
            ...
            try {
                getDataRest = await CallAPI(
                    'GET',
                    `{{ url('') }}/api/internal/admin-panel/perusahaan/pengajuan`, {
                        id: id,
                        page: page,
                        limit: limit,
                        ascending: ascending,
        ...
        async function getListLaporanTahunan(limit = 10, page = 1, ascending = 0, search = '', id = referenceId) {
            ...
            const getDataRest = await CallAPI(
                'GET',
                `{{ url('') }}/api/internal/admin-panel/perusahaan/laporan`, {
                    id: id,
                    page: page,
                    limit: limit,
                    ascending: ascending,
        ...
        async function getListKbli(limit = 10, page = 1, ascending = 0, search = '', id = referenceId) {
            if (is_active_kbli === 1 || is_active_kbli === true) {
                ...
                const getDataRest = await CallAPI(
                    'GET',
                    `{{ url('') }}/api/internal/admin-panel/perusahaan/kbli`, {
                        id: id,
                        page: page,
                        limit: limit,
                        ascending: ascending,
